...
/// - `none`: Raw codex, user's own settings (nomadterm may not work)
...
/// Ensure --add-dir ~/.nomadterm is present so nomadterm can write to its DB.
///
/// Codex's --add-dir flag is IGNORED in read-only sandbox mode, but required
/// for workspace-write mode to allow nomadterm DB writes.
///
...
/// Add nomadterm bootstrap to codex developer_instructions.
///
...
/// Preprocess Codex CLI arguments for nomadterm integration.
///
/// Applies:
/// 1. Sandbox flags based on mode
/// 2. --add-dir ~/.nomadterm for nomadterm DB writes

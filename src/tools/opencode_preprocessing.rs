//! OpenCode launch preprocessing — sets environment variables for nomadterm integration.
//! Plugin management is handled separately in hooks/opencode.rs.

use std::collections::HashMap;

/// Auto-approve JSON for nomadterm bash commands.
/// Value for OPENCODE_PERMISSION env var.
const OPENCODE_PERMISSION_JSON: &str = r#"{"bash":{"nomadterm *":"allow"}}"#;
...
mod tests {
    ...
    fn test_preprocess_sets_permission() {
        ...
        assert!(perm.contains("nomadterm *"));
    ...
    fn test_permission_json_is_valid() {
        ...
        assert!(parsed["bash"]["nomadterm *"].is_string());
    }
}
